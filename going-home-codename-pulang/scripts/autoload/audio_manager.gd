...
func unlock() -> void:
	...
	if DisplayServer.get_name() == "headless":
		return
	unlocked = true
	for player in [idle, load_loop, wind, rain, birds]:
		player.play()
...
func _process(delta: float) -> void:
	...
	rain.volume_db = lerpf(rain.volume_db, -20.0 if rain_target > 0 else -60.0, delta * 2)
	birds.volume_db = lerpf(birds.volume_db, -34.0 if rain_target > 0 else -24.0, delta * 2)
